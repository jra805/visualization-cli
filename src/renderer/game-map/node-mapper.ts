...
const LOCATION_NAMES: Record<string, Record<string, string>> = {
  ...
  decorator: {
    small: "Sigil",
    medium: "Rune Chamber",
    large: "Inscription Hall",
  },
  serializer: {
    small: "Scroll",
    medium: "Translator",
    large: "Bureau of Records",
  },
  ...
  return nodes.map((n) => {
    ...
    const importance = metrics
      ? pr * 40 +
        bt * 15 +
        (d.isGodModule ? 25 : 0) +
        (mt === "entry-point" || mt === "page" || mt === "layout" ? 10 : 0)
      : (d.fanIn + d.fanOut) * 3 +
        Math.log2(d.loc + 1) * 2 +
        (d.isGodModule ? 25 : 0);
    ...
    const biome: BiomeType = d.isCircular
      ? "volcanic"
      : MODULE_BIOME[mt] || "plains";
    ...
    if (d.isHotspot && !threats.some((t) => t.type === "hotspot")) {
      threats.push({
        type: "hotspot",
        severity: (d.hotspotScore ?? 0) >= 0.75 ? "error" : "warning",
        message: "Hotspot (score: " + (d.hotspotScore ?? 0).toFixed(2) + ")",
      });
    }
    if (d.isOrphan && !threats.some((t) => t.type === "orphan-module")) {
      threats.push({
        type: "orphan-module",
        severity: "info",
        message: "Orphaned module — no imports or exports",
      });
    }
    if (
      d.isCircular &&
      !threats.some((t) => t.type === "circular-dependency")
    ) {
      threats.push({
        type: "circular-dependency",
        severity: "error",
        message: "Part of circular dependency cycle",
      });
    }
    if (d.isGodModule && !threats.some((t) => t.type === "god-module")) {
      threats.push({
        type: "god-module",
        severity: "warning",
        message: "God module — too many dependencies or too large",
      });
    }
