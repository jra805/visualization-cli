...
import { packCircles, boundingCircle } from "../src/renderer/svg/circle-packing.js";
import { buildSvg, type SvgCircleData } from "../src/renderer/svg/svg-builder.js";
...
describe("boundingCircle", () => {
  it("contains all packed circles", () => {
    ...
    for (const c of packed) {
      const dist = Math.sqrt((c.x - bounds.cx) ** 2 + (c.y - bounds.cy) ** 2) + c.r;
...
describe("buildSvg", () => {
  function makeCircleData(): SvgCircleData[] {
    return [
      { id: "a", x: 0, y: 0, r: 30, label: "auth", moduleType: "service", language: "typescript" as Language, loc: 100, directory: "src" },
      { id: "b", x: 70, y: 0, r: 20, label: "utils", moduleType: "util", language: "javascript" as Language, loc: 50, directory: "src" },
      { id: "c", x: 0, y: 60, r: 15, label: "config", moduleType: "config", loc: 20, directory: "src", isHotspot: true, hotspotScore: 0.8 },
    ];
  }
  ...
  it("colors by module type by default", () => {
    const svg = buildSvg(makeCircleData(), { cx: 20, cy: 20, r: 80 }, { colorBy: "type" });
    expect(svg).toContain("#CF8C5C"); // service color
    expect(svg).toContain("#8E99A4"); // util/config color
  });

  it("colors by language when specified", () => {
    const svg = buildSvg(makeCircleData(), { cx: 20, cy: 20, r: 80 }, { colorBy: "language" });
...
describe("generateSvg", () => {
  function makeGraph(): Graph {
    ...
    nodes.set("/p/a.ts", { id: "/p/a.ts", filePath: "/p/a.ts", label: "a", moduleType: "component", loc: 100, directory: "src", language: "typescript" as Language });
    nodes.set("/p/b.ts", { id: "/p/b.ts", filePath: "/p/b.ts", label: "b", moduleType: "util", loc: 50, directory: "src", language: "typescript" as Language });
    return { nodes, edges: [] };
  }

  it("generates complete SVG from graph", () => {
    const svg = generateSvg(makeGraph(), {
      totalModules: 2, totalEdges: 0, issues: [], circularDeps: [], orphans: [], topCoupled: [],
    });
