...
describe("renderer", () => {
  ...
  describe("serializer", () => {
    it("converts graph Map to array with computed flags", () => {
      const graph = createGraph();
      addNode(graph, { id: "src/a.ts", filePath: "src/a.ts", label: "a", moduleType: "component", loc: 20, directory: "src" });
      addNode(graph, { id: "src/b.ts", filePath: "src/b.ts", label: "b", moduleType: "hook", loc: 15, directory: "src" });
      addNode(graph, { id: "src/c.ts", filePath: "src/c.ts", label: "c", moduleType: "util", loc: 5, directory: "src" });
      addEdge(graph, { source: "src/a.ts", target: "src/b.ts", type: "import" });
      addEdge(graph, { source: "src/b.ts", target: "src/a.ts", type: "import" });

      const report = makeReport({
        circularDeps: [["src/a.ts", "src/b.ts"]],
        orphans: ["src/c.ts"],
        issues: [{ type: "god-module", severity: "warning", message: "test", files: ["src/a.ts"] }],
      });
      ...
      const circularEdge = result.edges.find(e => e.data.source === "src/a.ts")!;
      expect(circularEdge.data.isCircular).toBe(true);
    });

    it("joins component and dataFlow info to nodes", () => {
      const graph = createGraph();
      addNode(graph, { id: "src/App.tsx", filePath: "src/App.tsx", label: "App", moduleType: "component", loc: 30, directory: "src" });

      const components: ComponentInfo[] = [
        { name: "App", filePath: "src/App.tsx", props: [{ name: "title", type: "string", isRequired: true }], hooksUsed: ["useState"], childComponents: ["Header"], isDefaultExport: true },
      ];
      const dataFlows: ComponentDataFlow[] = [
        { componentName: "App", filePath: "src/App.tsx", dataSources: [{ type: "api", name: "fetch", detail: "fetch('/api')" }] },
      ];
  ...
  describe("interactive html", () => {
    ...
    it("includes issue flags in data blob", () => {
      ...
      const report = makeReport({ circularDeps: [["x.ts"]], orphans: ["x.ts"] });
